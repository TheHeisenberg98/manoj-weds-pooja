...
import { signUpWithEmail, signInWithGoogle, getSession } from '@/lib/auth';
...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (authError) {
      ...
      setSuccess(true);
      setLoading(false);
      // Try redirecting — if email confirmation is disabled, session will exist
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        >
          {success ? (
            <div className="text-center py-4">
              ...
              <h2 className="text-lg font-light mb-2" style={{ color: '#F5E6D0' }}>Account Created!</h2>
              <p className="text-sm mb-6" style={{ color: '#A89A8C' }}>
                Check your email for a confirmation link, then sign in.
              </p>
              <Link
                href="/login"
                className="inline-block px-8 py-3 rounded-xl text-sm font-semibold tracking-wider uppercase transition-all hover:scale-105"
                style={{
        ...
        {!success && (
          <p className="text-center mt-6 text-sm" style={{ color: '#A89A8C' }}>
            ...
            <Link href="/login" className="underline transition-colors hover:opacity-80" style={{ color: '#D4A853' }}>
              Sign in
            </Link>
          </p>
        )}
